...
@Component({
  ...
  template: `
    <form (submit)="submit($event)">
      <h3 class="text-lg font-bold mb-4">{{ article() ? 'Edit article' : 'New article' }}</h3>
      ...
      <fieldset class="fieldset gap-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="fieldset-label">Unit price (CHF) *</label>
          ...
          <div>
            <label class="fieldset-label">VAT override (%)</label>
        ...
        <div>
          <label class="fieldset-label">Stock quantity</label>
...
export class ArticleFormComponent {
  ...
  // linkedSignal: resets to the source value when article() changes
  ...
  protected readonly errors = computed(() => ({
    name: this.name().trim() === '' ? 'Name is required' : null,
    unitPrice: (() => {
